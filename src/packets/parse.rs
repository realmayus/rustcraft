...
pub(crate) async fn parse_packet(
    ...
    let packet: Box<dyn ServerPacket> = match (id.value, &connection.state()) {
        ...
        (0x03, ConnectionState::Configuration) => {
            Box::new(server::ConfigurationKeepAlive::read(&mut read_from, connection).await?)
        }
        ...
        (0x17, ConnectionState::Play) => {
            Box::new(server::SetPlayerPositionAndRotation::read(&mut read_from, connection).await?)
        }
